\thusetup{
  ...
  %
  % 关键词用“英文逗号”分割
  ckeywords={Vlasov-Poisson, 适定性，爆破性},
  ekeywords={Vlasov-Poisson, well-posedness, blow up}
}
...
\begin{cabstract}
  ...
  % 而等离子体物理领域著名的朗道阻尼现象，在本文介绍中也略有提及其相关的数学工作成果。
  ...
  % 第三章给出了非相对论情形和相对论情形的全局解的存在性问题相关结果的整理。对于相对论的 Vlasov-Poisson 问题，在初值紧支集的假设下，证明了 $\mu=1$ 的情况解具有全局存在性，而 $\mu=-1$  足够小的初值时解具有全局存在性，而在较大的情况下则不然，解不能全局延拓。
...
% The well-known Landau damping, a varitey of long-term time asymptotic behaviour of Vlasov-Poisson system, is introduced concisely before the main body.
